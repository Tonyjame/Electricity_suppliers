...
public class AppNewsDetail {
    ...
    private String action; //动作：开始加载=1 加载成功=2 加载失败=3 推出页面=4
    private String goodsid; //商品ID，服务端下发的ID
    private String showtype; //商品样式：0 无图 1：一张大图 2：两张图 3：三张小图
    //4:一张小图 5：一张大图两张小图 6：来源于详情页相关推荐的商品，上报样式都为0 因为都是左文右图
    private String news_staytime; //页面停留时长：从商品开始加载时开始计算，到用户
    //关闭页面所用的时间，若中途用跳转到其他页面了，则暂停计时，待回到详情页时恢复计时
    //或中途划出的时间超过10分钟 则本次计时作废，计算页面开始加载到接口返回数据的时间
